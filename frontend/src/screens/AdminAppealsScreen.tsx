...
function AppealList() {
  ...
  useEffect(() => { setData(null); void api.admin.appeals({ page, size: 20, status: status || undefined, overdue: overdue || undefined }).then(setData).catch((e) => setError(e instanceof Error ? e.message : String(e))); }, [page, status, overdue]);
...
function AppealDetailView({ appealId }: { appealId: number }) {
  ...
  const [error, setError] = useState<string | null>(null);
  useEffect(() => { void api.admin.appeal(appealId).then(setDetail).catch((e) => setError(e instanceof Error ? e.message : String(e))); }, [appealId]);
  async function process() { setBusy(true); setError(null); try { await api.admin.processAppeal(appealId, { decision, note: note.trim() || undefined }); navigate("/admin/appeals", { replace: true }); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } }
  return <AdminFrame title="이의 상세"><button className="back" onClick={() => navigate(-1)}>‹</button>{!detail ? <p className={`muted${error ? "" : " loading"}`}>{error ?? "불러오는 중입니다"}</p> : <><div className="admin-heading"><div><p className="eyebrow">이의 #{detail.appealId}</p><h1>{detail.nickname ?? `회원 #${detail.memberId}`}</h1></div>{detail.overdue && <span className="admin-badge overdue">기한 초과</span>}</div><div className="admin-detail-grid"><section><h2>퇴출 정보</h2><Info label="세션" value={`#${detail.eviction.sessionId}`} /><Info label="퇴출 시각" value={formatDate(detail.eviction.evictedAt)} /><Info label="경고" value={`${detail.eviction.warningCount}회`} /><Info label="차감 포인트" value={`${detail.eviction.pointPenalty}P`} /></section><section><h2>신청자 진술</h2><div className="admin-note"><p>{detail.reasonText}</p><small>{formatDate(detail.createdAt)}</small></div></section><section className="admin-wide"><h2>경고 근거</h2><div className="admin-table-wrap"><table><thead><tr><th>#</th><th>근거</th><th>지속</th><th>전송 지연</th><th>동시 미검출</th></tr></thead><tbody>{detail.warnings.map((w) => <tr key={w.seq}><td>{w.seq}</td><td>{w.basis === "ABSENCE" ? "자리비움" : "화장실 초과"}</td><td>{w.absentSeconds == null ? "-" : `${w.absentSeconds}초`}</td><td>{w.reportSkewSeconds == null ? "-" : `${w.reportSkewSeconds}초`}</td><td>{w.concurrentReporterCount ?? "-"} / {detail.sessionParticipantCount}</td></tr>)}</tbody></table></div></section></div>{detail.status === "PENDING" && <section className="admin-action"><h2>심사 결정</h2><div className="admin-filters"><select value={decision} onChange={(e) => setDecision(e.target.value as typeof decision)}><option value="ACCEPTED">이의 인용</option><option value="REJECTED">이의 기각</option></select></div><textarea rows={4} maxLength={500} value={note} onChange={(e) => setNote(e.target.value)} placeholder="판단 근거를 기록하세요." />{error && <p className="error">{error}</p>}<button className="cta" disabled={busy} onClick={process}>심사 확정</button></section>}</>}</AdminFrame>;
}
