...
| 완주일의 귀속 | `completed_on` = **세션이 시작한 날**(`live_session.started_at`의 날짜). 종료일이 아니다 | 종료일을 쓰면 자정을 넘긴 세션이 다음 날로 넘어가, 23:30에 시작한 60분 세션은 시작한 날에 기록을 남기지 않는다. 심야에 매일 공부하는 사람의 연속이 매일 끊긴다(실측). 시작일 기준이어도 하루 한 행 제약은 그대로 성립한다 |
...
**경고 수는 구간의 길이에 비례한다.** 임계(`absence-threshold-seconds`)를 넘긴 구간은 1회이고, 그 뒤로는 `absence-warning-escalation-seconds`를 넘길 때마다 1회씩 더 붙는다. 즉 `1 + floor((지속초 − 임계 − 1) / 눈금)`이며 임계 이하는 0회다. 한 번의 보고로 `warningCount`가 2 이상 오를 수 있고, 퇴출 상한(`evict-warning-count`)에 닿으면 그 자리에서 멈춘다 — 상한을 넘겨 쌓지 않는다.

구간 하나당 1회이던 이전 규칙은 **지속시간이 아니라 빈도를 처벌**했다. 60분 세션에서 45분을 비운 사람이 경고 1회로 완주해 포인트와 Streak을 받고, 61초짜리 이석을 세 번 한 사람이 퇴출에 −300P를 무는 역전이 실측으로 재현됐다.

눈금을 임계와 같게 두지 않은 이유는 **3분 남짓 한 번에 곧바로 퇴출**이 되기 때문이다. 택배·전화처럼 흔하고 악의 없는 이석이 그 길이이고, 자발적으로 쓰는 서비스에서 첫 사건에 −300P와 재매칭 금지를 물리면 단계적 경고(D11)라는 설계가 사라진다. 눈금을 넓게 두면 **한 구간만으로 퇴출되는 지점이 화장실 모드 상한(10분)보다 뒤로** 밀린다 — 이 서비스가 "잠깐 자리를 비워도 되는 시간"으로 스스로 정해 둔 값이 10분이므로 설명할 수 있는 선이다. 기본값 60초/300초에서 그 지점은 **11분 1초**다.

등급이 갈리는 계단은 임계 앞뒤 1초에만 있고 그것은 기존 규칙 그대로다. 그 뒤 계단은 눈금만큼 벌어져 있어 훨씬 완만하다.

이 계산은 **경고를 만드는 세 자리가 모두 같은 것을 쓴다** — SS-4 판정, SS-5 Pause 시작 마감, §5 종료 정산. 갈리면 같은 자리비움이 도착 경로에 따라 다르게 판정된다. (SS-6 Pause 초과 복귀는 자리비움이 아니라 화장실 상한 축이라 언제나 1회다. D9)
