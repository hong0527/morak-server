...
- **세션 안에서 남에게 일어난 일** — `GET /api/sessions/{id}`(SS-1) 폴링. 남이 나가거나
  퇴출되거나 화장실 모드에 들어간 것, 그리고 **세션이 조기 종료된 것**까지 전부 이 경로로만
  안다. 내 요청의 응답에는 아무것도 실리지 않는다. 2~3초 간격을 권한다(§3 실례 ③).
...
> 아래는 "어느 API를 부르는가"까지다. **폴링·상태 전이·끊김 복구까지 포함해 이 화면을
> 끝까지 설계한 예가 §3 끝의 "실례 — 세션 화면 하나를 끝까지 만들어 본다"에 있다.**
> 이 화면을 실제로 만든다면 그쪽을 함께 읽는다.
...
**`expiresInSeconds`가 3600인데 세션은 240분까지 있다.** 토큰 수명이 세션보다 짧을 수 있다는
뜻이라, **재접속할 때 저장해 둔 토큰을 재사용하면 안 된다.** SS-2를 다시 부르면 새 토큰이
나온다(세션이 `LIVE`이고 내가 `ACTIVE`/`PAUSED`인 동안 몇 번이든 재발급된다).
...
### 실례 — 세션 화면 하나를 끝까지 만들어 본다

위 S1~S13은 "어느 API를 부르는가"까지다. 화면 하나를 실제로 설계하면 **그 목록만으로는
답이 안 나오는 질문**이 남는다. 가장 복잡한 세션 화면(S7)으로 그 질문들을 끝까지 따라간다.
다른 화면은 이 예를 보고 유추하면 된다.

#### ① 화면에 무엇이 올라가는가

| 요소 | 출처 |
|---|---|
| 내 캠 · 남의 캠 5개 | LiveKit (서버 아님) |
| 남은 시간 카운트다운 | SS-1 `endsAt` |
| 참가자 카드 6개 (닉네임 · 상태 · 경고 수 · 오늘 할 일) | SS-1 `participants[]` |
| 내 경고 배지 | SS-4/SS-5/SS-6 응답의 `warningCount` |
| 화장실 모드 버튼과 남은 시간 | SS-5 `resumeDueAt` · SS-1 `pauseUsed` |
| 스티커 3종 | `GET /api/stickers`, 전송은 LiveKit 데이터 채널 |
| 나가기 | SS-7 |

여기서 이미 갈린다. **캠은 LiveKit이 주고, 그 밖의 거의 전부는 SS-1이 준다.** 두 축이
서로를 모르므로 `identity`(= `memberId` 문자열)로 맞추는 것이 화면의 첫 번째 일이다.

#### ② 입장 — 순서가 정해져 있다

```
GET  /api/sessions/{id}          → 참가자·시각·roomName
POST /api/sessions/{id}/token    → url·token·identity
그다음 LiveKit 연결
```

SS-2를 먼저 부르면 안 되는 이유가 있다. **SS-2는 캠 분석 동의가 없으면 403
`CONSENT_REQUIRED`로 막힌다.** 그때 화면은 S3(동의)로 보내야 하는데, SS-1을 먼저 불러
두면 그 사이에 참가자 목록을 이미 그려 둘 수 있어 동의 후 복귀가 매끄럽다.

SS-2가 막히는 경로가 넷이다. 전부 세션 화면에서 일어난다.

| 응답 | 뜻 | 화면 |
|---|---|---|
| 403 `CONSENT_REQUIRED` | 캠 분석 미동의 | S3으로 |
| 409 `SESSION_ENDED` | 이미 끝난 세션 | 결과 화면(S8)으로 |
| 409 `ALREADY_EVICTED` | 내가 퇴출됨 | 결과 화면으로 (`evictionId`는 거기서) |
| 403 `NOT_SESSION_PARTICIPANT` | 내가 나간 세션이거나 남의 세션 | 홈으로 |

**한 번 나가면(LEFT) 다시 못 들어온다.** 재입장 경로가 없어서 참가자가 아닌 것과 같은
응답으로 막힌다.

#### ③ 그린 다음 — SS-1을 계속 다시 물어야 한다

**이것이 문서에서 가장 놓치기 쉬운 자리다.** §2-1이 "서버는 아무것도 밀어 주지 않는다"고
했고, 거기 적힌 폴링 대상은 매칭 성사뿐이다. 그래서 SS-1을 입장할 때 한 번만 부르고 마는
화면을 만들기 쉽다. **그러면 세션 내내 아무것도 갱신되지 않는다.**

SS-1을 다시 부르지 않으면 화면이 모르는 것들이다.

| 사건 | SS-1의 무엇이 바뀌나 |
|---|---|
| 남이 자율 퇴장했다 | 그 참가자 `status`가 `LEFT` |
| 남이 퇴출됐다 | 그 참가자 `status`가 `EVICTED`, `warningCount` 3 |
| 남이 화장실 모드에 들어갔다 | 그 참가자 `paused: true`, `status` `PAUSED` |
| 남이 오늘 할 일을 적었다 | 그 참가자 `goalText` |
| 아직 안 들어온 사람이 들어왔다 | 그 참가자 `joinedAt`이 채워짐 |
| **세션이 조기 종료됐다** | `status`가 `ENDED`, `endReason`이 `EARLY_UNDER_MIN` |

**2~3초 간격을 권한다.** 매칭 대기(S6)와 같은 값이고, 서버가 강제하지는 않는다.

> 폴링 주기가 짧으면 서버가 먼저 지친다. 2코어 서버에서 동시 조회 30이면 CPU가 포화한다
> (`deployment.md` §8). 대기자와 세션이 늘면 주기를 늘리는 쪽이 서버를 키우는 것보다 싸다.

`joinedAt`이 `null`인 것은 **매칭은 됐지만 아직 LiveKit 룸에 들어오지 않은 참가자**다.
서버가 SS-10 웹훅으로 최초 1회만 채운다. 입장 직후에는 내 것도 `null`일 수 있다 —
웹훅이 도착하기 전이라서다. 이 값으로 "접속 중"을 그리려면 그 지연을 감안해야 한다.

`goalText`는 **남의 것도 그대로 내려온다.** 서로의 "오늘 할 일"을 보는 것이 이 서비스의
화면 의도다(가리지 않는다). 다만 파기된 회원의 행은 `null`로 비워져 있다.

#### ④ 상태 전이 세 가지 — 화면이 무엇을 알고 무엇을 다시 그리는가

**(가) 내가 경고를 받았다**

경고를 만드는 경로는 셋뿐이고 **셋 다 응답에 실려 온다**(§2-1). 따로 물어볼 API가 없다.

| 경로 | 응답 필드 |
|---|---|
| SS-4 자리비움 보고 | `warningCount` · `evicted` · `evictionId` · `pointDelta` |
| SS-5 화장실 모드 시작 | `warningIssued` · `warningCount` · `closedAbsenceSeconds` |
| SS-6 복귀 | `warningIssued` · `warningCount` · `evicted` |

**SS-4는 카메라 모듈이 부른다(§5).** 그러니 그 응답을 화면이 받으려면 두 코드 사이에
경로가 있어야 한다. 카메라 모듈이 SS-4를 보내고 화면은 폴링으로 알아내는 구조로 짜면,
경고 배지가 최대 폴링 주기만큼 늦게 뜬다 — 그리고 **퇴출은 그보다 심각하다.**

> **경계를 이렇게 잡기를 권한다.** 카메라 모듈이 SS-4 응답을 그대로 화면에 콜백으로
> 넘기고, 화면이 `warningCount`·`evicted`를 해석한다. 카메라 모듈은 "얼굴이 보이나"만
> 알고 화면 전환은 모르는 편이 낫다. 레퍼런스 구현(`frontend/ai-detection/`)의
> `src/client/controller.ts`가 이 콜백 자리를 이미 열어 두었다.

`evicted: true`가 오면 그 자리에서 세션이 끝난 것이다. `evictionId`를 들고 결과
화면(S8)으로 보내고, LiveKit 연결을 끊는다. **서버가 캠을 끊어 주지 않는다**(§6-3) —
화면이 직접 끊지 않으면 퇴출된 사람의 영상이 계속 흐른다.

**(나) 다른 사람이 나갔다**

알 방법은 SS-1 폴링뿐이다. 남이 나가거나 퇴출돼도 **내 요청의 응답에는 아무것도 실리지
않는다.** LiveKit 쪽에서 참가자 이탈 이벤트가 먼저 올 수도 있는데, 그것과 서버의
`status`는 별개다 — LiveKit에서 사라졌어도 서버는 아직 `ACTIVE`일 수 있다(끊김과 퇴장은
다르다. (다)를 본다).

**참가자 카드를 지우지 않는다.** SS-1은 `LEFT`·`EVICTED`도 목록에 그대로 포함시킨다.
빈자리가 드러나야 하기 때문이고, 화면도 카드를 남기고 상태만 바꾸는 편이 의도에 맞다.

**(다) 세션이 조기 종료됐다**

참가자가 **2명 미만**이 되면 세션은 그 자리에서 끝난다. 남은 사람에게는 아무 예고가 없다.

트리거는 "누가 빠지는 모든 경로"다 — 자율 퇴장 · 퇴출 · 재접속 유예 초과. 배치를 기다리지
않고 그 트랜잭션 안에서 즉시 끝난다.

**화장실 모드는 자리를 뜬 것으로 세지 않는다.** `PAUSED`도 남아 있는 인원에 포함되므로,
둘만 남은 세션에서 한 명이 화장실 모드에 들어가도 세션은 끝나지 않는다.

화면이 아는 방법은 **SS-1 폴링에서 `status`가 `ENDED`, `endReason`이 `EARLY_UNDER_MIN`으로
바뀌는 것**뿐이다. 그때 결과 화면으로 보낸다.

> **조기 종료여도 남아 있던 사람은 완주다.** 30분 만에 끝난 60분 세션도 남은 사람에게는
> `targetMinutes` 기준 포인트가 그대로 나간다 — 일찍 끝난 것이 그 사람 책임이 아니라서다.
> 결과 화면에 "중단됨"으로 그리면 사실과 다르다.

#### ⑤ 끊겼을 때 — 90초와 1시간

두 개의 시간 제한이 있고 **문서 어디에도 같이 적혀 있지 않아 놓치기 쉽다.**

**90초 — 재접속 유예.** LiveKit 연결이 끊기면 서버가 90초를 기다린다. 그 안에 돌아오면
없던 일이 되고, 넘기면 `LEFT(DEVICE_ISSUE)`로 정산된다. **미완주가 되고 되돌릴 수 없다.**
새로고침·화면 전환으로 연결이 끊기는 구조라면 이 창 안에 반드시 다시 붙어야 한다.

**1시간 — LiveKit 토큰 수명.** SS-2가 주는 토큰은 `expiresInSeconds: 3600`이다. 그런데
**세션은 최대 240분까지 있다.** 4시간짜리 세션에서 3시간째에 새로고침하면 **처음 받은
토큰은 이미 만료돼 있다.**

그래서 **재접속할 때는 저장해 둔 토큰을 쓰지 말고 SS-2를 다시 부른다.** 재발급은 허용되고
(세션이 `LIVE`이고 내가 `ACTIVE`/`PAUSED`인 동안), 그때마다 새 유효 기간이 붙는다.
비용이 거의 없으므로 **새로고침 복구 경로는 항상 SS-1 → SS-2 → LiveKit 순서로 다시 타는
것**이 안전하다.
...
| 길 | 어떻게 알게 되나 | 다음 화면 |
|---|---|---|
| 정시 종료 | `endsAt` 경과 후 SS-1 `status`가 `ENDED` (최대 1분 지연, §2-2) | 결과(S8) |
| 조기 종료 | SS-1 `status` `ENDED` + `endReason` `EARLY_UNDER_MIN` | 결과(S8) |
| 퇴출 | SS-4/SS-6 응답의 `evicted: true` | 결과(S8) → 이의(S9) |
| 자율 퇴장 | 내가 SS-7을 불러 204를 받음 | 홈(S4) |

**넷 중 셋이 결과 화면으로 모인다.** 그리고 결과 화면은 §2-2 때문에 최대 1분간 409를
돌려준다. 즉 세션 화면에서 결과 화면으로 넘어가는 길목에는 **"정산 중" 상태가 반드시
필요하다.** 이걸 만들지 않으면 정시 종료 직후 사용자가 오류 화면을 본다.

#### ⑦ 이 화면에서 자주 틀리는 것

- SS-1을 입장할 때 한 번만 부른다 → 남이 나가도 조기 종료돼도 화면이 그대로 멈춰 있다
- 저장해 둔 LiveKit 토큰으로 재접속한다 → 1시간이 지난 세션에서 실패한다
- 퇴출 응답을 받고 LiveKit 연결을 안 끊는다 → 퇴출된 사람 캠이 계속 흐른다
- 결과 화면의 409를 오류로 처리한다 → 정시 종료 직후 항상 보인다
- `LEFT` 참가자 카드를 목록에서 지운다 → 빈자리가 안 보인다
- 화장실 모드 버튼을 계속 활성으로 둔다 → 두 번째는 409 `PAUSE_ALREADY_USED`다.
  `pauseUsed`로 미리 막는다
- SS-7 본문을 빠뜨린다 → `DELETE`인데 사유가 필수다(400)
...
설계와 근거는 **`docs/ai-pipeline.md`**, 구현은 **`frontend/ai-detection/`** 폴더에 있다.
...
1. **보내는 시각은 판단이 끝난 시각이 아니라 얼굴이 실제로 사라진 프레임의 시각이다.**
   서버가 그 간격으로 구간의 지속 초를 재고 그 초에서 경고 수를 뽑기 때문에, 판단 지연이
   시각에 섞이면 판정이 통째로 밀린다.
...
4. **401은 재시도하지 않는다.** 큐를 보존한 채 전송만 멈추고, 재로그인해 새 토큰을 받으면
   재개한다. 토큰 만료는 백오프로 풀리지 않아서, 네트워크 오류로 흘리면 세션이 끝날 때까지
   아무도 모른 채 두드리기만 한다. 큐를 버리는 것도 안 된다 — 그동안의 자리비움이 사라진다.
   감지는 계속 돌려 이벤트를 큐에 쌓아 둔다.

### `clientSeq` 순번은 세 겹으로 지킨다

순번을 유실하고 1부터 다시 시작하면 서버가 전부 중복으로 보고, 클라이언트는 409를 성공으로
취급하므로 **그 세션의 감지가 조용히 통째로 꺼진다.** 한 겹으로는 못 막는다.

1. **`localStorage`에 보존한다. `sessionStorage`가 아니다.** 세션 저장소는 탭 단위라
   새로고침만 넘긴다 — 탭을 닫았다 다시 들어오거나, 새 탭에서 열거나, 브라우저를
   재시작하거나, iOS가 메모리 압박으로 탭을 폐기하면 전부 비어 있다. 4시간짜리 세션에서
   탭이 닫히는 것은 평범한 일이라 실제로 재현됐다.
2. **저장값과 무관한 바닥을 둔다.** 번호의 하한을 "세션 시작으로부터 흐른 초"로 잡는다.
   저장소를 바꿔도 기기·브라우저를 갈아타면 여전히 비어 있기 때문이다. 세션 시작 시각은
   SS-1이 내리는 값을 그대로 넘긴다. 앞선 실행이 5초 페이싱으로 쓸 수 있는 번호는 경과 초의
   1/5뿐이라, 나중에 시작한 쪽의 바닥이 항상 더 커서 겹치지 않는다.
3. **처음 보낸 번호에 409가 오면 번호를 다시 매겨 재전송한다.** 두 번 이상 보낸 번호의
   409는 내 재전송이 흡수된 것이라 성공이지만, 한 번도 보낸 적 없는 번호의 409는 다른
   탭·기기가 이미 쓴 번호라는 뜻이고 **내 이벤트는 서버에 기록되지 않았다.** 여기서 성공으로
   취급하면 자리비움이 사라진다.
